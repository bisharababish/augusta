...
import {
  ...
  BarChart3, LogOut, Globe, HeartPulse, Menu, X,
} from "lucide-react";
...
function AppLayout() {
  ...
  // Fix #14 — useIsMobile is now actually used to auto-close sidebar on mobile navigation
  ...
  return (
    <div dir={dir} className="min-h-screen flex bg-soft">
      {/* Sidebar */}
      <aside
        className={`fixed lg:sticky top-0 z-40 h-screen w-64 bg-sidebar text-sidebar-foreground flex flex-col transition-transform duration-200 ${open ? "translate-x-0" : dir === "rtl" ? "translate-x-full" : "-translate-x-full"
      ...
      >
          ...
          {NAV.map(({ to, labelKey, icon: Icon }) => {
            ...
            return (
              <Link
                key={to}
                to={to}
                className={`flex items-center gap-3 rounded-lg px-3 py-2.5 text-sm transition-all ${active
        ...
        <div className="p-3 border-t border-sidebar-border">
          <div className="flex items-center gap-3 px-2 py-2 rounded-lg">
            <div className="h-9 w-9 rounded-full bg-sidebar-primary text-sidebar-primary-foreground flex items-center justify-center text-sm font-bold shrink-0">
              {getDisplayName(user, lang).charAt(0)}
            </div>
            <div className="flex-1 min-w-0">
              <div className="text-sm font-medium truncate">{getDisplayName(user, lang)}</div>
              <div className="text-xs opacity-70 truncate capitalize">{t(user.role as never)}</div>
            </div>
          </div>
          <Button
            variant="ghost"
            size="sm"
            className="w-full justify-start mt-2 text-sidebar-foreground hover:bg-sidebar-accent hover:text-sidebar-primary-foreground"
